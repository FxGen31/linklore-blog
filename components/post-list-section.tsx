...
export default function PostListSection({
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(
      (post) =>
        post.keywords.includes(currentCategory) || currentCategory === 'all'
    );
    ...
    >
      <header className='flex flex-col'>
        ...
        <span className='mb-8 mt-2 block max-w-3xl text-base font-normal text-muted-foreground sm:text-xl md:mb-10 md:mt-3'>
          ...
          molestias voluptates aut eveniet repellat explicabo accusamus est
          incidunt corrupti!
        </span>
      ...
      <div className='mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2 '>
        {displayingPosts.length > 0 && (
          <>
            <FeaturedPostCard post={displayingPosts[0]} className='lg:mb-16' />
            <div className='grid gap-8 lg:grid-rows-3'>
              {displayingPosts
                .filter((_, i) => i < 4 && i > 0)
                .map((post, idx) => (
                  <RegularPostCard key={`${post.title}-${idx}`} post={post} />
                ))}
            </div>
          </>
        )}
